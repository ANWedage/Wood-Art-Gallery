...
import RequestedMaterial from './RequestedMaterial';
...
export default function InventoryDashboard() {
  ...
  const initialPOForm = { itemName:'', quantity:'', boardSize:'', thickness:'', color:'', description:'', unitPrice:0, totalPrice:0 };
  ...
  const updateStockPrices = async () => {
    try {
      setStockLoading(true);
      setStockError(null);
      const res = await fetch('/api/stock/update-prices', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        alert(`Successfully updated prices for ${data.updatedCount} stock items!`);
        loadStockItems();
        loadStockSummary();
      } else {
        setStockError(data.message || 'Failed to update stock prices');
      }
    } catch (e) {
      setStockError('Error updating stock prices: ' + e.message);
    } finally {
      setStockLoading(false);
    }
  };
  ...
  const onPOChange = async (e) => {
    ...
    // If relevant fields changed, fetch unit price and calculate total
    const relevantFields = ['itemName', 'boardSize', 'thickness', 'color', 'quantity'];
    if (relevantFields.includes(name)) {
      const nextValues = { ...poForm, [name]: value };
      const { itemName, boardSize, thickness, color, quantity } = nextValues;
      
      if (itemName && boardSize && thickness && color) {
        try {
          const query = new URLSearchParams({ material: itemName, boardSize, thickness, color }).toString();
          const res = await fetch(`/api/stock/combination?${query}`);
          const data = await res.json();
          if (data.success && data.stock) {
            const unitPrice = data.stock.price || 0;
            const qty = Number(quantity) || 0;
            setPOForm(prev => ({ ...prev, unitPrice, totalPrice: unitPrice * qty }));
          } else {
            setPOForm(prev => ({ ...prev, unitPrice: '', totalPrice: '' }));
          }
        } catch (err) {
          setPOForm(prev => ({ ...prev, unitPrice: '', totalPrice: '' }));
        }
  ...
  const submitPO = async (e) => {
    ...
    const { itemName, quantity, boardSize, thickness, color, totalPrice } = poForm;
    ...
    try {
      ...
      const poPayload = { 
        supplierId: currentSupplierPO._id, 
        itemName, 
      ...
      if(poData.success){
        ...
        const paymentPayload = {
          supplierName: currentSupplierPO.name,
          supplierEmail: currentSupplierPO.email,
          item: itemName,
          color: color,
          size: boardSize,
          thickness: thickness,
        ...
        // Send to supplier payment table
        const paymentRes = await fetch('/api/supplier-payments', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if(paymentData.success) {
          console.log('Supplier payment record created:', paymentData.payment);
        } else {
          console.error('Failed to create supplier payment record:', paymentData.message);
        }
        
        setPurchaseOrders(list=>[poData.purchaseOrder, ...list]);
        setShowPOModal(false);
        setPOForm(initialPOForm);
        alert('Purchase order created and payment record added successfully!');
      } else {
        alert(poData.message || 'Failed to create purchase order');
      }
    } catch(err){ 
  ...
  const onSupplierChange = (e) => { 
    ...
    if (name === 'name') {
      if (/\d/.test(value)) {
        return; // Don't update if value contains numbers
      }
    }
    ...
    if (name === 'phone') {
      if (!/^\d*$/.test(value) || value.length > 10) {
        return; // Don't update if value contains non-digits or exceeds 10 digits
      }
    }
      ...
      <div className="inventory-dashboard-root">
        <aside className="inventory-sidebar">
          ...
          <ul>
            ...
            <li className={activePage === 'custom-order-management' ? 'active' : ''} onClick={() => goTo('custom-order-management')}>
              <div className="menu-icon">📦</div>
              <span>Custom Order Management</span>
            </li>
            <li className={activePage === 'requested-material' ? 'active' : ''} onClick={() => goTo('requested-material')}>
              <div className="menu-icon">📋</div>
